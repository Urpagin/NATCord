...
def poll():
    ...
    if get_history:
        ...
        messages = (
            Message.query.filter(Message.timestamp > last_dt)
...
def get_client_activity():
    ...
    active = {
        user_id: ts
        for user_id, ts in active_clients.items()
        if now - ts < active_threshold
    }
    active_ids = list(active.keys())

    # Query the database for users with these IDs.
    active_users = [
        User.query.get(user_id).username
...
def update_profile():
    ...
    if chat_color:
        if not re.fullmatch(r"#[0-9a-fA-F]{6}", chat_color):
            errors.append(
                "Invalid chat color format. Please use a hex code like #RRGGBB."
            )
    ...
    if icon_b64:
        if not icon_b64.startswith("data:image/"):
            return (
                jsonify({"error": "Invalid icon format. Please upload a valid image."}),
                400,
            )
